...
echo $dateNow;
$sql = '';
foreach ($Man->return() as $manager) {
    $sqlCheck = "SELECT id FROM " . CALLS . " WHERE date='$dateNow' AND name='{$manager['FIO']}'";
    ...
    if (!($sth->fetch()))
    {
        $sql .= "INSERT INTO " . CALLS . " (`date`, `name`, `count_in`, `time_in`, `count_out`, `time_out`) VALUES ('$dateNow','$name',$inCount,$inTime,$outCount, $outTime);";
    }
    else
    {
        $sql .= "UPDATE " . CALLS . " SET `count_in` = $inCount, `time_in` = $inTime, `count_out` = $outCount, `time_out` = $outTime WHERE date = '$dateNow' AND name = '$name';";
    }
}
echo $sql;
$pdo->exec($sql);